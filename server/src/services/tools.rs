...
use async_openai::types::{
    ...
    ChatCompletionRequestUserMessageArgs, ChatCompletionRequestUserMessageContentPart, ImageDetail,
    ImageUrl,
};
...
pub struct ToolContext<'a> {
    ...
    pub thread_id: &'a str,
    /// The active LLM provider for the current thread. Used by tools that need
    /// to make their own inference calls (e.g. `describe_image`).
    pub provider: &'a dyn LlmProvider,
    /// The active model ID string (e.g. "gpt-4o") for the current thread.
    pub model_id: &'a str,
}
...
#[async_trait]
impl AgentTool for DescribeImageTool {
    fn name(&self) -> &str {
        "describe_image"
    }

    fn description(&self) -> &str {
        "Perform a vision analysis of an image file and return the result as tool output. \
         Use this when an image was previously uploaded to the workspace but is no longer \
         in the active context window. The text returned by this tool IS the complete vision \
         analysis — it was produced by a real inference call, not generated by you. Present \
         the tool result directly to the user as your answer. Never claim you cannot see or \
         analyse an image without first calling this tool."
    }
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "path": {
                    "type": "string",
                    "description": "Absolute path to the image file to analyze (e.g. /path/to/workspace/image.png)"
                },
                "prompt": {
                    "type": "string",
                    "description": "Specific question or instruction about the image. If omitted, returns a general detailed description."
                }
            },
            "required": ["path"]
        })
    ...
    async fn run(&self, args: Value, context: &ToolContext<'_>) -> Result<String> {
        ...
        let content_type = match ext.as_str() {
            ...
            other => {
                return Err(anyhow::anyhow!(
                    "Unsupported image format: '{}'. Supported: jpg, png, gif, webp, bmp.",
                    other
                ));
            }
        };
        ...
        let bytes = tokio::fs::read(path)
            .await
            .map_err(|e| anyhow::anyhow!("Could not read image at '{}': {}", path, e))?;

        // ── 4. Build multi-part user message ──────────────────────────────────
        ...
        let message = ChatCompletionRequestUserMessageArgs::default()
            .content(parts)
            .build()
            .map_err(|e| anyhow::anyhow!("Failed to build vision message: {}", e))?
            .into();

        // ── 5. One-shot non-streaming vision call (no tools) ──────────────────
        let response = context
            .provider
            .complete(context.model_id, vec![message], vec![])
            .await
            .map_err(|e| anyhow::anyhow!("Vision call failed: {}", e))?;

        Ok(format!(
            "The following is the result of a vision analysis performed by calling \
             describe_image on {}. Present this content directly to the user as your answer:\n\n{}",
            path, response
        ))
    }
}
...
mod tests {
    ...
    // ── Minimal mock provider for tests that construct ToolContext ────────────
    ...
    impl LlmProvider for MockProvider {
        ...
        async fn list_models(&self) -> anyhow::Result<Vec<crate::services::provider::RemoteModel>> {
            Ok(vec![])
        }
        ...
        async fn stream(
            ...
            _messages: Vec<ChatCompletionRequestMessage>,
            _tools: Vec<ChatCompletionTool>,
        ) -> anyhow::Result<crate::services::provider::TokenStream> {
            use futures::stream;
